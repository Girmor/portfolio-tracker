-- Migration 10: Fix import_ibkr_commit to store user_id so RLS SELECT works
-- Previously, the procedure didn't set user_id on the imports record.
-- The service role bypasses INSERT RLS, but SELECT RLS (user_id = auth.uid())
-- filtered the record out — imports appeared to succeed but weren't visible.

CREATE OR REPLACE FUNCTION commit_ibkr_import(
  p_portfolio_id    uuid,
  p_filename        text,
  p_broker          text,
  p_trades          jsonb,
  ...
  p_summary         jsonb,
  p_user_id         uuid DEFAULT NULL   -- new param; NULL = legacy/service-role calls
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  -- Create import record — include user_id so RLS SELECT lets the user see it
  INSERT INTO imports (
  ...
  IF p_ending_cash IS NOT NULL THEN
    INSERT INTO cash_adjustments (
      portfolio_id, previous_balance, new_balance,
      date, notes
    ) VALUES (
...
-- Backfill: set user_id on existing import records that lack it.
-- Match via portfolio's user_id since we know portfolio_id.
UPDATE imports i
